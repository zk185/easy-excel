...
public class ExcelBuilderImpl implements ExcelBuilder {
    ...
    public void addContent(List data, int startRow, int startCol, int flag) {
        Row row;
        if (CollectionUtils.isEmpty(data)) {
            return;
        }
        if (flag == 0) {
            context.getCurrentSheet().shiftRows(startRow,context.getCurrentSheet().getLastRowNum(),data.size());
        }
        for (int i = 0; i < data.size(); i++) {
            int rowNum = i + startRow;
            if (flag == 0) {
                row = context.getCurrentSheet().createRow(rowNum);
            } else {
                row = context.getCurrentSheet().getRow(rowNum);
            }
            addOneRowOfDataToExcel(data.get(i), row, startCol);
        }
    }
    ...
    @Override
    public void addContent(List data, Sheet sheetParam, int startRow, int startCol, int flag) {
        context.currentSheet(sheetParam);
        addContent(data, startRow, startCol, flag);
    }
    ...
    private void addBasicTypeToExcel(List<Object> oneRowData, Row row, int startCol) {
        if (CollectionUtils.isEmpty(oneRowData)) {
            return;
        }
        for (int i = 0; i < oneRowData.size(); i++) {
            int colNum = i + startCol;
            Object cellValue = oneRowData.get(i);
            Cell cell = WorkBookUtil.createCell(row, colNum, context.getCurrentContentStyle(), cellValue,
                    TypeUtil.isNum(cellValue));
            if (null != context.getAfterWriteHandler()) {
                context.getAfterWriteHandler().cell(i, cell);
            }
    ...
    private void addJavaObjectToExcel(Object oneRowData, Row row, int startCol) {
        int i = startCol;
        BeanMap beanMap = BeanMap.create(oneRowData);
        for (ExcelColumnProperty excelHeadProperty : context.getExcelHeadProperty().getColumnPropertyList()) {
            BaseRowModel baseRowModel = (BaseRowModel)oneRowData;
    ...
    private void addOneRowOfDataToExcel(Object oneRowData, Row row, int startCol) {
        if (oneRowData instanceof List) {
            addBasicTypeToExcel((List)oneRowData, row, startCol);
        } else {
            addJavaObjectToExcel(oneRowData, row, startCol);
        }
    }
}
